...
import com.wms.panchkula.databinding.ActivitySaleTransferLineBinding
...
import com.wms.panchkula.ui.saleToDelivery.DeliveryDraftDocumentLinesActivity
...
class SaleTransferLinesActivity : AppCompatActivity(), PassList,
    SaleTransferItemAdapter.AdapterCallback {
    companion object {
    ...
    private lateinit var activityFormBinding: ActivitySaleTransferLineBinding

    //    private lateinit var PurchaseTransferItemAdapter: DemoAdapter
    private lateinit var PurchaseTransferItemAdapter: SaleTransferItemAdapter
    ...
    private var selectedSaleToInvoiceType = ""

    val items = listOf("Sales to Invoice as Draft"/*, "Sales to Invoice as Document"*/)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        activityFormBinding = ActivitySaleTransferLineBinding.inflate(layoutInflater)
        ...
        materialProgressDialog = MaterialProgressDialog(this@SaleTransferLinesActivity)
        sessionManagement = SessionManagement(this@SaleTransferLinesActivity)
    ...
    private fun setSaleToInvoiceTypeAdapter() {
        val adapter = ArrayAdapter(this, R.layout.drop_down_item_textview, items)
        activityFormBinding.acSalesToInvoiceType.setAdapter(adapter)

        activityFormBinding.acSalesToInvoiceType.setOnItemClickListener { _, _, position, _ ->
            val selectedItem = items[position]
            selectedSaleToInvoiceType = selectedItem
            activityFormBinding.acSalesToInvoiceType.setText(selectedItem, false)
        }
    }
    ...
    fun setAdapter() {
        ...
        PurchaseTransferItemAdapter = SaleTransferItemAdapter(
            this@SaleTransferLinesActivity,
    ...
    private fun postInventorystock(inventoryItem: PurchaseRequestModel.Value, list: MutableList<PurchaseRequestModel.StockTransferLines>) {
        ...
        if (selectedSaleToInvoiceType.equals("Sales to Invoice as Draft", ignoreCase = true)) {
        ...
        postedJson.addProperty("U_WMSUSER", sessionManagement.getUsername(this@SaleTransferLinesActivity))  //WMS userName tagged (added by Vinod @25Apr,2025)
...
//                        commit by tarun
            ...
            val networkClient = NetworkClients.create(this@SaleTransferLinesActivity)
            val quantityNetworkClient = QuantityNetworkClient.create(this@SaleTransferLinesActivity)
            if (selectedSaleToInvoiceType.equals("Sales to Invoice as Draft", ignoreCase = true)) {
                quantityNetworkClient.ARDraft_Posting(postedJson).apply {
                ...
                networkClient.postSalesInvoiceDocument(postedJson).apply {
                    enqueue(object : Callback<PurchasePostResponse> {
                        override fun onResponse(
                            call: Call<PurchasePostResponse>,
                            response: Response<PurchasePostResponse>
                            ...
                            try {
                                ...
                                AppConstants.IS_SCAN = false
                                materialProgressDialog.dismiss()
                                DeliveryDraftDocumentLinesActivity.staticString = ""
                                Log.e("success---BP---", "==>" + response.code())
                                if (response.code() == 201 || response.code() == 200) {
                                    Log.e("success------", "Successful!")
                                    Log.d(
