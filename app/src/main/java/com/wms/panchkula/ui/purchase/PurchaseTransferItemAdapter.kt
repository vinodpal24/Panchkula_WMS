...
import com.wms.panchkula.databinding.PurchaseTransferLinesAdapterLayoutBinding
...
class PurchaseTransferItemAdapter(
    private val context: Context, var list: ArrayList<PurchaseRequestModel.StockTransferLines>, private val networkConnection: NetworkConnection,
    private val materialProgressDialog: MaterialProgressDialog, private val callback: AdapterCallback, private val save: Chip, private val taxList: ArrayList<TaxListModel.Value>
) : RecyclerView.Adapter<PurchaseTransferItemAdapter.ViewHolder>(), BatchItemsAdapter.OnDeleteItemClickListener {
...
//        setSqlServer()
    ...
    interface AdapterCallback {
        ...
        fun onApiResponseStock(
            ...
            list: ArrayList<PurchaseRequestModel.StockTransferLines>,
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val binding = PurchaseTransferLinesAdapterLayoutBinding.inflate(LayoutInflater.from(parent.context), parent, false)
    ...
    override fun onBindViewHolder(holder: ViewHolder, @SuppressLint("RecyclerView") position: Int) {
        with(holder) {
            with(list[position]) {
                ...
                // list[position].totalOpenDefault = list[position].RemainingOpenQuantity
                binding.venView.visibility = View.GONE
                binding.qrTitle.text = "Num Card :"
                binding.tvTotalScanQtyLabel.text = "GRPO Qty"
                ...
                val staticValue = PurchaseTransferLinesActivity.staticString

                binding.edBatchCodeScan.setText(staticValue)
                tvTotalScanQty = binding.tvTotalScannQty

                setFreightTaxSpinner(binding, taxList, this.TaxCode, list[bindingAdapterPosition])
                /* if (list[position].totalOpenDefault > 0.0) {
                     val newTint = ContextCompat.getColorStateList(context, R.color.grey)  // Get the tint color
                     holder.binding.mainCard.backgroundTintList = newTint
                 } else {
                     val newTint = ContextCompat.getColorStateList(context, R.color.login_background)  // Get the tint color
                     holder.binding.mainCard.backgroundTintList = newTint
                 }*/

                if (list[position].totalOpenDefault > 0.0) {
                    val drawable = ContextCompat.getDrawable(context, R.drawable.bg_docline_item_selected_grey)
                    holder.binding.mainLayout.background = drawable
                } else {
                    val drawable = ContextCompat.getDrawable(context, R.drawable.bg_docline_item)
                    holder.binding.mainLayout.background = drawable
                }
                ...
                binding.tvTotalScanGw.text = this.ItemType//"$totalScanGW"
                ...
                binding.mainCard.setOnClickListener {
                    ...
                    callSystemBinByWarehouse(this.WarehouseCode, context, position, binding.tvTotalScannQty, list[position].binAllocationJSONs)
                    Log.e("DataList=>" + position, list[position].binAllocationJSONs.toString())
                    //openDynamicFieldsDialog(context, position, binding.tvTotalScannQty, list[position].binAllocationJSONs)
                }
  ...
  */
                ...
                else if (sessionManagement.getScannerType(context) == "QR_SCANNER" || sessionManagement.getScannerType(context) == null) {
...
//                            pos = adapterPosition
                ...
                save.setOnClickListener {
                    if (binding.edBatchCodeScan.text.equals("0.0")) {
                        ...
                        PurchaseTransferLinesActivity.staticString = binding.edBatchCodeScan.text.toString()
                        /* for (i in 0 until list.size) {
                        ...
                        Log.d("BP-List-11=>", list.toString())
    ...
    private fun callSystemBinByWarehouse(warehouseCode: String, context: Context, position: Int, tvTotalScannQty: TextView, binItems: ArrayList<PurchaseRequestModel.binAllocationJSONs>) {
        ...
        networkClient.getSystemBinByWarehouse(warehouseCode).apply {
            enqueue(object : Callback<SystemBinModel> {
                override fun onResponse(call: Call<SystemBinModel>, response: Response<SystemBinModel>) {
                    try {
                        if (response.isSuccessful) {
                            materialProgressDialog.dismiss()
                            var responseModel = response.body()!!
                            openDynamicFieldsDialog(context, position, tvTotalScannQty, binItems, responseModel)
                        } else {
                            materialProgressDialog.dismiss()
                ...
                override fun onFailure(call: Call<SystemBinModel>, t: Throwable) {
...
//                                Prefs.clear()
    ...
    private fun setFreightTaxSpinner(binding: PurchaseTransferLinesAdapterLayoutBinding, freightTaxList: ArrayList<TaxListModel.Value>, taxCode: String, stockTransferLines: PurchaseRequestModel.StockTransferLines) {
        val taxList = freightTaxList.map { it.Tname }
        val adapter = ArrayAdapter(context, R.layout.drop_down_item_textview, taxList)

        binding.acTaxCode.setAdapter(adapter)
        // Find matching tax object
        val matchedTax = freightTaxList.find { it.Tcode == taxCode }

        if (matchedTax != null) {
            binding.acTaxCode.setText(matchedTax.Tname, false) // <-- show readable Tname in AutoCompleteTextView

            selectedFreightTaxCodeName = matchedTax.Tname.toString()
            selectedFreightTaxCode = matchedTax.Tcode.toString()
            selectedFreightTaxPercentage = matchedTax.Rate.toString()
            stockTransferLines.TaxCode = selectedFreightTaxCode
            stockTransferLines.RateField = selectedFreightTaxPercentage
            Log.d("TAX_MATCH", "Matched Tcode: $selectedFreightTaxCode, Name: ${matchedTax.Tname}")
        } else {
            binding.acTaxCode.setText("")
            Log.e("TAX_MATCH", "No tax code matched for: $taxCode")
        }

        // Optional: Item click listener
        binding.acTaxCode.setOnItemClickListener { parent, view, position, id ->
    ...
    //TODO viewholder...
    class ViewHolder(val binding: PurchaseTransferLinesAdapterLayoutBinding) : RecyclerView.ViewHolder(binding.root)
...
//                                                    val mapValue = listCopy.firstOrNull()?.toDoubleOrNull()?.toInt()
                                                ...
                                                batchItemsAdapter?.setOnDeleteItemClickListener(this@PurchaseTransferItemAdapter)
                                                rvBatchItems.adapter = batchItemsAdapter
...
//                            }
    ...
    private fun openDynamicFieldsDialog(context: Context, po: Int, tvTotalScannQty: TextView, binItems: ArrayList<PurchaseRequestModel.binAllocationJSONs>, responseModel: SystemBinModel) {
        ...
        val systemBinList = responseModel.value
        ...
        Log.e("BIN_SELECTION", "BinItem List: $binItems")
        ...
        var scanTP = list[po].ScanType
        if (list[po].ItemType.equals("BATCH", true) && list[po].ScanType.equals("N", true)) {
            scanTP = "YB"
        }
        ...
        if (list[po].BinManaged.equals("N", true)) {
            ...
            setFieldAdapters(context, rvDynamicField, temp, scanTP, list[po].BinManaged, temp)

        } else {
            if (systemBinList.isEmpty()) {
                val binAbs = ArrayList<String>()
                val binLocationList = ArrayList<String>()
                setFieldAdapters(context, rvDynamicField, binLocationList, scanTP, list[po].BinManaged, binAbs)
            } else {
                var binAbs = ArrayList<String>()
                var binLocationList = ArrayList<String>()
                systemBinList.forEach {
                    binAbs.add(it.AbsEntry)
                     binLocationList.add(it.Code)
                }

                Log.i(
                    "BIN_SELECTION",
                    "Popup => BinAbsEntry: $binAbs, BinLocation: $binLocationList}"
                )
        ...
        btnSave.setOnClickListener {
            ...
            list[po].grpoGrandTotal = list[po].Price.toDouble() * totalQty

            val totalGRPOAmount = list.sumOf { it.grpoGrandTotal }
            Prefs.putString(AppConstants.GRPO_TOTAL, "$totalGRPOAmount")

            //tvGrpoAmount.text = totalGRPOAmount.toString()

            Log.e("BinData=New==>" + po, "Total Qty: $totalQty Price: ${list[po].Price} GRPO Total: ${list[po].grpoGrandTotal} Total Amount: $totalGRPOAmount")
            Log.e("BinData=New==>" + po, list[po].binAllocationJSONs.toString())
            Log.e("BinData=New==>" + po, list[po].binAllocationJSONs.size.toString())
            dialog.dismiss()
            binLocation.clear()
            notifyDataSetChanged()
