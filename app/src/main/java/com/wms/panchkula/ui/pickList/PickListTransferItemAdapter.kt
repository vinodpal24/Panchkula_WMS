...
class PickListTransferItemAdapter(private val context: Context, var list: ArrayList<PurchaseRequestModel.StockTransferLines>, private val networkConnection: NetworkConnection,
                                  private val materialProgressDialog: MaterialProgressDialog, private val callback: AdapterCallback, private val save: Chip) : RecyclerView.Adapter<PickListTransferItemAdapter.ViewHolder>(), BatchItemsAdapter.OnDeleteItemClickListener {
    ...
    override fun onBindViewHolder(holder: ViewHolder, @SuppressLint("RecyclerView") position: Int) {
        with(holder) {
            with(list[position]) {
                ...
                val staticValue = PickListTransferLinesActivity.staticString
...
//                            pos = adapterPosition
                ...
                save.setOnClickListener {
                    if(binding.edBatchCodeScan.text.equals("0.0")){
                        ...
                        PickListTransferLinesActivity.staticString = binding.edBatchCodeScan.text.toString()
                       /* for (i in 0 until list.size) {
...
//                                                    val mapValue = listCopy.firstOrNull()?.toDoubleOrNull()?.toInt()
                                                ...
                                                batchItemsAdapter?.setOnDeleteItemClickListener(this@PickListTransferItemAdapter)
                                                rvBatchItems.adapter = batchItemsAdapter
