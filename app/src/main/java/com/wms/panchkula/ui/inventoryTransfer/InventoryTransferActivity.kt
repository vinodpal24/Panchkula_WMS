...
import com.wms.panchkula.databinding.ActivityInventoryTransferBinding
...
class InventoryTransferActivity : AppCompatActivity(), GoodsItemLineAdapter.OnDeleteItemClickListener, GoodsChanchalAdapter.OnItemActionListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityInventoryTransferBinding.inflate(layoutInflater)
...
//        getPostJson()
        ...
        materialProgressDialog = MaterialProgressDialog(this@InventoryTransferActivity)
        checkNetwoorkConnection = CheckNetwoorkConnection(application)
        sessionManagement = SessionManagement(this@InventoryTransferActivity)
        ...
        binding.ivAdd.setOnClickListener {
            ...
            // Creating a blank LocalListForGoods object
            val blankItem = LocalListForGoods(
                DocEntry = 0, // Providing a default value for non-nullable Int
                ItemCode = "", // Default empty string for non-nullable String
                ItemDescription = "", // Default empty string for non-nullable String
                Batch = "", // Default empty string for nullable String
                SystemNumber = 0, // Default value for non-nullable Int
                SerialNumber = null, // Default null for nullable String
                ScanType = "", // Default empty string
                Quantity = "", // Default empty string
                FixedQuantity = "", // Default empty string
                WareHouseCode = "", // Default empty string
                UnitPrice = "", // Default empty string
                BatchNumber = null, // Default null for nullable String
                SystemSerialNumber = 0L, // Default value for non-nullable Long
                InternalSerialNumber = null, // Default null for nullable String
                NoneVal = null, // Default null for nullable String
                wareHouseListing = mutableListOf() // Default empty list
            )
            //  AppConstants.scannedItemForGood.add(blankItem)
            //  goodsOrderLineAdapter!!.notifyDataSetChanged()


            /*  val inflater = LayoutInflater.from(this)
              val newMainView = inflater.inflate(R.layout.activity_inventory_transfer,  binding.parentLayout, false)

              // Add the newly created TableLayout to the parent layout
              binding.parentLayout.addView(newMainView)*/
        }
        ...
        binding.rvBatchItems.apply {
            ...
            layoutManager = LinearLayoutManager(this@InventoryTransferActivity)
        ...
        if (sessionManagement.getScannerType(this@InventoryTransferActivity) == "LEASER") {
        ...
        else if (sessionManagement.getScannerType(this@InventoryTransferActivity) == "QR_SCANNER" || sessionManagement.getScannerType(this@InventoryTransferActivity) == null) { //|| sessionManagement.getLeaserCheck()!! == 0 && sessionManagement.getQRScannerCheck()!! == 1 || sessionManagement.getLeaserCheck()!! == 0 && sessionManagement.getQRScannerCheck()!! == 0
...
//                itemCode = this.ItemNo.toString()
                ...
                if (sessionManagement.getScannerType(this@InventoryTransferActivity) == null) {
                    showPopupNotChooseScanner()
                } else if (sessionManagement.getScannerType(this@InventoryTransferActivity) == "QR_SCANNER") {
                    val intent = Intent(this@InventoryTransferActivity, QRScannerActivity::class.java)
                    (this@InventoryTransferActivity as InventoryTransferActivity).startActivityForResult(intent, REQUEST_CODE)
                }
        ...
        binding.etPostingDate.setOnClickListener {
            GlobalMethods.datePicker(this@InventoryTransferActivity, binding.etPostingDate)
        }
                        ...
                        if (response.isSuccessful) {
                            ...
                            try {
                                ...
                                if (mError.error.code.equals(400)) {
                                    GlobalMethods.showError(this@InventoryTransferActivity, mError.error.message.value)
                                }
                ...
                override fun onFailure(call: Call<ModelSeries>, t: Throwable) {
                    ...
                    when (t) {
                        is SocketTimeoutException -> {
                            GlobalMethods.showError(this@InventoryTransferActivity, "Connection timed out. Please try again.")
                        }

                        is IOException -> {
                            GlobalMethods.showError(this@InventoryTransferActivity, "Network error. Please check your internet connection.")
                        }

                        else -> {
                            GlobalMethods.showError(this@InventoryTransferActivity, "Something went wrong: ${t.localizedMessage}")
                        }
                    }
//                                Prefs.clear()
    ...
    private fun setSeriesSinner(value: List<ModelSeries.Value>) {
        ...
        val adapter = ArrayAdapter(
            this@InventoryTransferActivity,
    ...
    /* @RequiresApi(Build.VERSION_CODES.N)
     private fun getJsonArray(list: List<LocalListForGoods>, batcharraySize: Int): JsonArray {
         ...
         // Key: Pair(BinAbsEntry, BinActionType), Value: Quantity sum
         val quantityMap = mutableMapOf<Pair<String, String>, Double>()

         if (!list.isNullOrEmpty()) {
             for (item in list) {
                 // From warehouse
                 if (fromBinLocation.isNotEmpty()) {
                     val keyFrom = Pair(fromBinLocation, "batFromWarehouse")
                     quantityMap[keyFrom] = quantityMap.getOrDefault(keyFrom, 0.0) + (item.Quantity.toDoubleOrNull() ?: 0.0)
                 }

                 // To warehouse
                 if (toBinLocation.isNotEmpty()) {
                     val keyTo = Pair(toBinLocation, "batToWarehouse")
                     quantityMap[keyTo] = quantityMap.getOrDefault(keyTo, 0.0) + (item.Quantity.toDoubleOrNull() ?: 0.0)
                 }
             }
         }

         // Convert map entries to JsonObjects and add to JsonArray
         for ((key, qty) in quantityMap) {
             val (binAbsEntry, binActionType) = key
             val obj = JsonObject()
             obj.addProperty("BinAbsEntry", binAbsEntry)
             obj.addProperty("Quantity", qty)
             obj.addProperty("BinActionType", binActionType)
             obj.addProperty("SerialAndBatchNumbersBaseLine", baseLine)
             stockBin.add(obj)
         }
    ...
    private fun getPostJson() {
        ...
        if (networkConnection.getConnectivityStatusBoolean(applicationContext)) {
            if (fromWhareHouse.isNullOrEmpty()) {
                GlobalMethods.showError(this@InventoryTransferActivity, "Please scan fromWarehouse first.")
                return
            } else if (toWhareHouse.isNullOrEmpty()) {
                GlobalMethods.showError(this@InventoryTransferActivity, "Please scan toWarehouse first.")
                return
            } else if (selectedSeries.isNullOrEmpty()) {
                GlobalMethods.showError(this@InventoryTransferActivity, "Please select document series.")
                return
            }else if (binding.etPostingDate.text.toString().isNullOrEmpty()) {
                GlobalMethods.showError(this@InventoryTransferActivity, "Please select doc date.")
                return
            }
...
//                                    addProperty("WareHouseCode", item.WareHouseCode)
            ...
            val finalJsonObject = JsonObject().apply {
                ...
                addProperty("U_WMSUSER", sessionManagement.getUsername(this@InventoryTransferActivity))  //WMS userName tagged (added by Vinod @25Apr,2025)
            ...
            val networkClient = NetworkClients.create(this@InventoryTransferActivity)
                    ...
                    override fun onResponse(
                        ...
                        try {
                            ...
                            if (response.code() == 201 || response.code() == 200) {
                                ...
                                //GlobalMethods.showSuccess(this@InventoryTransferActivity, "Post Successfully. " + response.body()!!.DocNum.toString())
                                showSuccessDialog(
                                    context = this@InventoryTransferActivity,
            ...
            materialProgressDialog.dismiss()
            handler.post {
                Toast.makeText(this@InventoryTransferActivity, "No Network Connection", Toast.LENGTH_SHORT).show()
            }
        }
            ...
            if (requestCode == REQUEST_CODE) {
                ...
                if (result!!.isNotEmpty()) {
                    ...
                    if (type == "Batch") {
                        ...
                        GlobalMethods.showMessage(this@InventoryTransferActivity, "Scan Type is " + type)
                    }
    ...
    private fun showPopupNotChooseScanner() {
        val builder =
            AlertDialog.Builder(this@InventoryTransferActivity, R.style.CustomAlertDialog).create()
        val view =
            LayoutInflater.from(this@InventoryTransferActivity).inflate(R.layout.custom_popup_alert, null)
        ...
        yesBtn.setOnClickListener {
            var intent = Intent(this@InventoryTransferActivity, HomeActivity::class.java)
...
//todo use when call bind fun.
    ...
    private fun scanBatchLinesItem(text: String, rvBatchItems: RecyclerView, position: Int, itemCode: String?, tvTotalScannQty: TextView, type: String) {
        if (networkConnection.getConnectivityStatusBoolean(this)) {
                            ...
                            try {
                                ...
                                if (response.isSuccessful) {
                                    ...
                                    if (responseModel.value.size > 0 && !responseModel.value.isNullOrEmpty()) {
                                        ...
                                        GlobalMethods.showError(this@InventoryTransferActivity, "Invalid Batch Code")
            ...
            materialProgressDialog.dismiss()
            Toast.makeText(this@InventoryTransferActivity, "No Network Connection", Toast.LENGTH_SHORT)
                .show()
        }
    }
    ...
    private fun getQuantityForSuggestion(
        ...
        if (networkConnection.getConnectivityStatusBoolean(this@InventoryTransferActivity)) {
            materialProgressDialog.show()
            var apiConfig = ApiConstantForURL()
            ...
            val networkClient = QuantityNetworkClient.create(this@InventoryTransferActivity)
            networkClient.getQuantityForSuggestion(
                sessionManagement.getCompanyDB(this@InventoryTransferActivity)!!,
                remQt,
                itemCode,
                wareHouseCode
                                ...
                                if (response.isSuccessful) {
                                    ...
                                    if (responseModel.value.size > 0 && !responseModel.value.isNullOrEmpty()) {
                                        if (responseModel.value[0].Quantity.toString().trim().toDouble() > 0.0) {
                                            AppConstants.showAlertDialog(this@InventoryTransferActivity, responseModel.value[0].Batch, responseModel.value[0].Quantity)
                                        ...
                                        GlobalMethods.showError(this@InventoryTransferActivity, "No Quantity")
                                        Log.e("not_response---------", response.message())
                                    }
...
//                                    binding.tvTotalScannQty.text = ": $totalQuantity"
                                    ...
                                    if (stringList.isNotEmpty() && !stringList.contains("0")) {
                                        ...
                                        //  batchItemsAdapter?.notifyDataSetChanged()
                                        GlobalMethods.showError(this@InventoryTransferActivity, "Batch / Roll No. has zero Quantity of this PO.")
                                    }
                                } else {
                                    GlobalMethods.showError(this@InventoryTransferActivity, "No Quantity Found of this Goods Issue.")
                                    //   batchItemsAdapter?.notifyDataSetChanged()
                                }
                            } else {
    ...
    private fun handleErrorResponse(response: Response<GetQuantityModel>) {
        ...
        try {
            ...
            errorModel.error.message.value?.let {
                GlobalMethods.showError(this@InventoryTransferActivity, it)
                Log.e("json_error------", it)
            }
        } catch (e: IOException) {
            e.printStackTrace()
